...
Casks that require [`requires_rosetta`](Cask-Cookbook.md#caveats-requires_rosetta) are still acceptable while the latest major macOS release supports Rosetta 2 for general app compatibility. Apple says in its [Rosetta 2 transition documentation](https://developer.apple.com/documentation/apple-silicon/about-the-rosetta-translation-environment/) that this support will remain available through macOS 27, with only a narrower legacy-games-focused subset beyond that, so we expect to stop accepting new `requires_rosetta` casks when macOS 27 is the latest stable macOS version.
...
* Casks that do not work on the latest macOS version will be rejected. If Apple keeps its [current Rosetta 2 transition plan](https://developer.apple.com/documentation/apple-silicon/about-the-rosetta-translation-environment/), this will include x86-only software when macOS 28 is the latest macOS version.
