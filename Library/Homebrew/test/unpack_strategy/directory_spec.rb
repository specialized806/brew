...
RSpec.describe UnpackStrategy::Directory do
  ...
  test_each([false, true]) do |move|
    context "with `move: #{move}`" do
      it "does not follow symlinks" do
        described_class.new(path, move:).extract(to: unpack_dir)
      ...
      it "fails when overwriting a directory with a file" do
        ...
        strategy = described_class.new(path, move:)
  ...
  context "with `move: true`" do
    subject(:strategy) { described_class.new(path, move: true) }
